...
def runRace():
    ...
    for i in series:
        ...
        finalPlaces.append((i[0], series.index(i)+1))
    scores = []
    ...
    for i in scores:
        finalScores.append((scores.index(i) + 1, i[0], i[1]))
    
    return list(((finalPlaces),(finalScores)))




seriesSummary = runRace()
seriesPlacement = seriesSummary[0]
seriesPlacement = sorted(seriesPlacement, key=itemgetter(0))
seriesScore = seriesSummary[1]
print(sorted(seriesScore, key=itemgetter(2)))
results = {'Alice':[],
            'Clare':[],
            'Bob':[],
            'Dennis':[],
            'Eva':[]} 
for name,score in seriesPlacement:
